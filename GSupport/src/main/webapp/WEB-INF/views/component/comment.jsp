			...
			<li id="commentContainer">
				<div id="commentbox">
					<div id="commentInfo">
						...
						<span class="commentModify"> <c:if
								test="${sessionScope.id eq cl.member_id }">
								<a class="clear">수정하기</a>
							</c:if> <c:if
								test="${sessionScope.id eq cl.member_id || sessionScope.grade eq '3' }">
								<a href="#"
									onclick="return comment_delete(${cl.board_cno}, ${param.categoryNo }, ${detail.board_no })">삭제하기</a>
							</c:if> <input type="hidden" id="cNum" value="true" />
						</span>
						<p class="commentInfoText" style="color: gray; font-size: small;">
							<fmt:formatDate var="now" value="${today}" pattern="yyyyMMdd" />
							<fmt:formatDate var="write_date" value="${cl.comment_date}"
								pattern="yyyyMMdd" />
							<c:choose>
								<c:when test="${write_date eq now}">
									<fmt:formatDate value="${cl.comment_date}" pattern="HH:mm" />
								</c:when>
								<c:otherwise>
									<fmt:formatDate value="${cl.comment_date}" pattern="yyyy.MM.dd" />
								</c:otherwise>
							</c:choose>
						</p>
					</div>
					...
					<div class="modifyWrite">
						<form action="./comment_update.do?board_cno=${cl.board_cno }"
							method="post">
							<textarea name="ccontent" id="commentWriteText"
								placeholder="수정 내용 입력하세요. "></textarea>
		...
		<c:when test="${sessionScope.id ne null }">
			...
			<div id="commentWrite">
				<form action="./commentWrite.do" method="post">
					<textarea id="commentWriteText" name="comment_content"
						placeholder="댓글을 입력해주세요. 타인을 비방하는 댓글, 욕설, 도배행위 등은 제재대상이 될 수 있습니다."></textarea>
